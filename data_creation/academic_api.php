...
function createSession($conn) {
    ...
    $conn->query("
        ...
        SELECT `level`,program_id,students_count FROM levels WHERE students_count>0
    ");
    $conn->query("
        CREATE TEMPORARY TABLE max_levels AS
        SELECT program_id,MAX(`level`) AS max_level FROM levels GROUP BY program_id
    ");
    $conn->query("
        UPDATE levels l
